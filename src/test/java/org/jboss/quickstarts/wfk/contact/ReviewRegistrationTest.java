...
public class ReviewRegistrationTest {
...
//                .importRuntimeAndTestDependencies()
    ...
    public void testRegister() throws Exception {
    	...
    	Review review = createReviewInstance(user.getId(), Long.valueOf(100), "good food","3");
    ...
    public void testInvalidRegister() {
    	Review review = createReviewInstance(Long.valueOf(1), Long.valueOf(100), "","");
        ...
        try {
            ...
            log.info("Invalid review register attempt failed with return code " + e.getStatus());
        }
    ...
    public void testDuplicateReview() throws Exception {
        ...
        try {
        	reviewRestService.createReview(createReviewInstance(user.getId(), Long.valueOf(100), "good food","3"));
            fail("Expected a RestServiceException to be thrown");
        } catch(RestServiceException e) {
        	System.out.println(e.getMessage());
            assertEquals("Unexpected response status", Response.Status.CONFLICT, e.getStatus());
            assertTrue("Unexpected error. Should be Unique review violation", e.getCause() instanceof UniqueReviewException);
            assertEquals("Unexpected response body", 1, e.getReasons().size());
            log.info("Duplicate review register attempt failed with return code " + e.getStatus());
        }

    }

    private Review createReviewInstance(Long user_id, Long restaurant_id, String review1, String rating) {
